...
async function continueInit() {
  ...
  detectRobloxVersion();
  startRunningPoll();
  logEntry('info', 'system', 'Roblox Account Manager started', { version: 'v1', accounts: accounts.length, platform: navigator.platform });
  ...
  try {
    api.checkUpdates().then(update => {
      if (update && update.hasUpdate) {
        const lbl = document.getElementById('update-version-label');
        if (lbl) lbl.textContent = `Version ${update.version} is available!`;
        const changelogEl = document.getElementById('update-changelog');
        if (changelogEl) {
          changelogEl.innerText = update.body || 'No changelog provided.';
        }
        ...
        if (btn) {
          btn.onclick = async () => {
            ...
            const res = await api.startUpdate(update.downloadUrl || update.url);
            if (res && res.ok) {
              btn.textContent = 'Installing...';
            } else {
              btn.disabled = false;
              btn.textContent = 'Retry Update';
              toast('Update download failed. Opening browser...', 'err');
              api.openExternal(update.url);
              closeModal('m-update');
            }
          };
        }
...
function applyTheme(name) {
  if (THEMES.indexOf(name) < 0) name = 'graphite';
...
function currentTheme() { try { return localStorage.getItem('ui-theme') || 'graphite'; } catch { return 'graphite'; } }
...
function toggleTheme() {
  setTheme(currentTheme() === 'light' ? 'graphite' : 'light');
}
(function() {
  let t;
  try {
    t = localStorage.getItem('ui-theme');
    if (!t) { const old = localStorage.getItem('theme'); t = (old === 'light') ? 'light' : 'graphite'; }
  } catch { t = 'graphite'; }
  setTheme(t || 'graphite');
})();
