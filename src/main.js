...
function nativeSrcPath() {
  return app.isPackaged
    ? path.join(process.resourcesPath, 'RAMHelper.cs')
    : path.join(__dirname, 'RAMHelper.cs');
}
function bundledNativeExePath() {
  return app.isPackaged
    ? path.join(process.resourcesPath, 'RAMHelper.exe')
    : path.join(__dirname, 'RAMHelper.exe');
}
function findCsc() {
...
// Resolves to the path of a usable RAMHelper.exe, or null if none could be
...
function ensureNativeHelper() {
  ...
  _nativeHelperPromise = (async () => {
    ...
    const outExe = path.join(app.getPath('userData'), 'RAMHelper.exe');
    try {
...
app.whenReady().then(async () => {
  if (process.platform === 'win32') app.setAppUserModelId('com.robloxaccountmanagerbypadawan985.app');
...
async function checkUpdates() {
  return new Promise((resolve) => {
    const url = 'https://api.github.com/repos/Padawan986/Roblox-Account-Manager-by-Padawan985/releases/latest';
    const req = https.get(url, {
      headers: { 'User-Agent': 'Roblox-Account-Manager-by-Padawan985-Updater' }
    }, res => {
      let data = '';
      res.on('data', c => data += c);
      res.on('end', () => {
        try {
          if (res.statusCode !== 200) { resolve({ hasUpdate: false }); return; }
          ...
          // Find the installer asset ending with .exe
          ...
          resolve({
            ...
            url: release.html_url || 'https://github.com/Padawan986/Roblox-Account-Manager-by-Padawan985/releases/latest',
            downloadUrl,
            body: release.body || ''
          });
        } catch {
...
ipcMain.handle('app:checkUpdates', async () => {
  try { return await checkUpdates(); } catch { return { hasUpdate: false }; }
});

ipcMain.handle('app:startUpdate', async (event, downloadUrl) => {
  const dest = path.join(os.tmpdir(), 'Roblox_Account_Manager_Setup.exe');
  try {
    ...
    // Launch the downloaded setup with NSIS silent switch '/S'
    const { spawn } = require('child_process');
    spawn(dest, ['/S'], { detached: true, stdio: 'ignore' }).unref();
    setTimeout(() => {
      app.quit();
    }, 12000); // 12 seconds to ensure installer starts fully before we exit
    return { ok: true };
  } catch (e) {
