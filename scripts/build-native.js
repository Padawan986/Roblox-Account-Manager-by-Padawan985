// Precompiles src/RAMHelper.cs -> src/RAMHelper.exe using the .NET
...
// RAMHelper.cs and compiles it once on first run as a fallback, and falls
...
const src = path.join(__dirname, '..', 'src', 'RAMHelper.cs');
const out = path.join(__dirname, '..', 'src', 'RAMHelper.exe');
...
if (!fs.existsSync(src)) {
  console.warn('[build-native] src/RAMHelper.cs not found; skipping.');
  process.exit(0);
}
...
console.log('[build-native] compiling RAMHelper.exe ...');
